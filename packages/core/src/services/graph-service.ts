...
export class GraphService extends EventEmitter {
  private client = getPrismaClient();
  ...
  async initialize(): Promise<Graph> {
    try {
      ...
      let dbGraph = await this.client.graph.findFirst({
        where: { name: 'Default Graph' }
      });

      if (!dbGraph) {
        // Create default graph if it doesn't exist
        dbGraph = await this.client.graph.create({
          data: {
            id: 'default',
            name: 'Default Graph',
            path: './graphs/default',
      ...
      console.log('Graph initialized:', this.currentGraph.name);
